...
const processStatistics = (start, end) => {
    const newList = [];
    data.forEach((item) => {
        ...
        if (date >= end && date <= start) {
            ...
            for (let i = 0; i < newList.length; i++) {
                if (item.websiteId == newList[i].websiteId) {
                    found = true;
                    newList[i].chats += item.chats
                    newList[i].missedChats += item.missedChats
                    break;
                }
            }
            if (!found) {
...
console.log(processStatistics(new Date(2019, 3, 13), new Date(2019, 3, 10)))
